...
const API_BASE = import.meta.env.PROD
  ? 'http://api.owlvision.app'
  : ''

const AUTH = {
  loginUrl: `${API_BASE}/v3/discord/login/url`,
  loginPost: `${API_BASE}/v3/discord/login`,
  me:        `${API_BASE}/v3/discord/@me`,
}

async function openAuthPopup(onSuccess: () => void) {
  // 1. Get the Discord OAuth URL from backend
  let discordUrl: string
  try {
    const res = await fetch(AUTH.loginUrl, { credentials: 'include' })
    if (!res.ok) return
  ...
  // 2. Wait for postMessage from AuthCallback page
  const messageHandler = async (event: MessageEvent) => {
    ...
    try {
      const res = await fetch(AUTH.loginPost, {
        method: 'POST',
        credentials: 'include',
  ...
  // Fallback: if popup closed without message (user closed manually)
