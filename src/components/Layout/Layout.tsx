...
async function openAuthPopup(onSuccess: () => void) {
  try {
    ...
    const handleMessage = (event: MessageEvent) => {
      // Проверяем, что сообщение с нашего же домена
      if (event.origin !== window.location.origin) return

      if (event.data === 'AUTH_SUCCESS') {
        window.removeEventListener('message', handleMessage)
        clearInterval(pollTimer)
        onSuccess() // Вызываем функцию обновления юзера (fetchUser)
      }
    }
    window.addEventListener('message', handleMessage)

    // 4. Запасной вариант: если окно просто закрыли руками или postMessage не сработал
    const pollTimer = setInterval(() => {
      if (popup.closed) {
        clearInterval(pollTimer)
        window.removeEventListener('message', handleMessage)
        onSuccess() // Пробуем обновить юзера на случай, если всё же успел залогиниться
      }
    }, 500)
