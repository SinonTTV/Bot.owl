...
import { AUTH, apiFetch, openAuthPopup } from '../../utils/auth'
import './Layout.css'
...
function Navbar() {
    ...
    const handleLogout = async () => {
        setMenuOpen(false)
        try {
            await apiFetch(`${AUTH.me.replace('/discord/@me', '/auth/logout')}`, { method: 'POST' })
        } catch {
        }
        setUser(null)
        localStorage.removeItem('owlbot_guild_id')
    }
                    ...
                    {user ? (
                        <div className="nav-user-wrap" ref={menuRef}>
                            ...
                            >
                                ...
                                <span className="nav-user-chevron" style={{ transform: menuOpen ? 'rotate(180deg)' : 'none' }}>▾</span>
                            </button>
                            {menuOpen && (
                                <div className="nav-user-dropdown">
                                    <div className="nav-user-dropdown-info">
                                        {avatarUrl && <img src={avatarUrl} alt={user.username} className="nav-dropdown-avatar" />}
                                        <div>
                                            <div className="nav-dropdown-name">{user.globalName || user.username}</div>
