...
const AUTH = {
  login: `${API_BASE}/v3/discord/login`,
  me:    `${API_BASE}/v3/discord/@me`,
}

function openAuthPopup(onSuccess: () => void) {
  const callbackUrl = `${window.location.origin}/auth-callback`
  const url = `${AUTH.login}?redirect=${encodeURIComponent(callbackUrl)}`
  const width = 500
  const height = 700
  ...
  const autoClose = setTimeout(finish, 15000)
}
